...
ROOT = Path("/Users/yoda/forProgramming/DK/skills/visual-explanations-workspace/iteration-2")

RUNS = [
    ("vm-container-comparison", 1, "with_skill"),
    ("vm-container-comparison", 1, "old_skill"),
    ("oauth-login-flow", 2, "with_skill"),
    ("oauth-login-flow", 2, "old_skill"),
    ("kubernetes-hierarchy", 3, "with_skill"),
    ("kubernetes-hierarchy", 3, "old_skill"),
]
...
def check_expectation(text: str, expectation: str) -> tuple[bool, str]:
    if expectation == "Markdown に Mermaid コードブロックが含まれる":
        ...
        return passed, "Found ```mermaid block." if passed else "No Mermaid code block found."
    if expectation == "本文に共通点と差分の両方への言及がある":
        has_common = "共通" in text
        has_diff = any(word in text for word in ["差分", "違い", "分かれ目"])
        return has_common and has_diff, f"common={has_common}, difference={has_diff}"
    if expectation == "図または補足で読み順（上から下 / 左から右など）が示されている":
        passed = any(word in text for word in ["上から下", "左から右", "読み方", "読み順"])
        return passed, "Found reading-order guidance." if passed else "No explicit reading-order guidance found."
    if expectation == "認可コードとアクセストークンの両方が本文または図に現れる":
        has_code = "認可コード" in text
        has_token = "アクセストークン" in text
        return has_code and has_token, f"auth_code={has_code}, access_token={has_token}"
    ...
    if expectation == "Markdown の表が含まれる":
        ...
        return passed, "Found markdown table syntax." if passed else "No markdown table syntax found."
...
def main() -> None:
    benchmark_runs = []
    for eval_name, eval_id, config_dir in RUNS:
        run_dir = ROOT / eval_name / config_dir
        outputs_dir = run_dir / "outputs"
        metadata = json.loads(read_text(ROOT / eval_name / "eval_metadata.json"))
        answer = read_text(outputs_dir / "answer.md")
        expectations = []
        ...
        grading = {
            ...
            "summary": {"passed": passed_count, "failed": failed, "total": total, "pass_rate": pass_rate},
            "execution_metrics": {
                "tool_calls": {},
                "total_tool_calls": 0,
                "total_steps": 0,
            ...
            "eval_feedback": {
                "overall": "Iteration 2 still uses lightweight assertions. Use the review viewer to judge whether same-layer alignment is genuinely clearer, especially for eval 1."
            },
        }
        ...
        benchmark_runs.append({
            "eval_id": eval_id,
            "eval_name": eval_name,
            "configuration": "with_skill" if config_dir == "with_skill" else "without_skill",
            "run_number": 1,
            "result": {
                "pass_rate": pass_rate,
                "passed": passed_count,
    ...
    benchmark = {
        ...
        "notes": [
            "Iteration 2 incorporates user feedback: same-layer comparison should remain visible instead of being split away.",
            "Assertions are intentionally lightweight and mainly ensure structural completeness; human review is the main signal for eval 1.",
            "Timing and token counts from subagent notifications were not recoverable; token column uses output character count as a placeholder.",
        ],
    }
